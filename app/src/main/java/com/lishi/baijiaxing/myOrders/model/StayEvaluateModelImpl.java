...
public class StayEvaluateModelImpl extends BaseModel implements StayEvaluateModel {
    public MyOrderService mMyOrderService;
    private int mPage = 0;
    private boolean isFirst = false;

    public StayEvaluateModelImpl() {
        mMyOrderService = (MyOrderService) getRetrofitManager().getHomeService(MyOrderService.class);
    }

    @Override
    public void loadOrderList(final StayEvaluateCallback callback, final int page) {
        ...
        mMyOrderService.getMyOrderList(LoginUtil.getInstance().getLogin().getData().getUid(), LoginUtil.getInstance().getLogin().getData().getToken()
                , LoginUtil.getInstance().getLogin().getData().getType(), "1", sPage, "2").subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
    ...
    @Override
    public void changeOrderStatus(final StayEvaluateCallback callback, String orderNumber, String status) {
        if (!LoginUtil.getInstance().isLogin()) {
    ...
    @Override
    public void deleteOrder(final StayEvaluateCallback callback, String oid) {
        if (!LoginUtil.getInstance().isLogin()) {
            callback.deleteOrderStatusFailed("未登录");
            return;
        }
        mMyOrderService.deleteOrder(LoginUtil.getInstance().getLogin().getData().getUid(), LoginUtil.getInstance().getLogin().getData().getToken()
                , LoginUtil.getInstance().getLogin().getData().getType(), oid).subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread()).subscribe(new Subscriber<SCOperation>() {
            @Override
            public void onCompleted() {
            ...
            @Override
            public void onError(Throwable e) {
                callback.deleteOrderStatusFailed(e.toString());
                Logger.d(e.toString());
            }
            ...
            public void onNext(SCOperation scOperation) {
                if (scOperation.getStatus().equals("204")) {
                    callback.deleteOrderStatusSuccess(scOperation);
                } else {
                    callback.deleteOrderStatusFailed(scOperation.getMsg());
                }
            }
        });
    }
