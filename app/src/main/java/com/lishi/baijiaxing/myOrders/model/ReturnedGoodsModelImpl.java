...
public class ReturnedGoodsModelImpl extends BaseModel implements ReturnedGoodsModel {
    public MyOrderService mMyOrderService;
    private int mPage = 0;
    private boolean isFirst = false;

    public ReturnedGoodsModelImpl() {
        mMyOrderService = (MyOrderService) getRetrofitManager().getHomeService(MyOrderService.class);
    }

    @Override
    public void loadOrderList(final ReturnedGoodsCallback callback, final int page) {
        ...
        mMyOrderService.getMyOrderList(LoginUtil.getInstance().getLogin().getData().getUid(), LoginUtil.getInstance().getLogin().getData().getToken()
                , LoginUtil.getInstance().getLogin().getData().getType(), "1", sPage, "3").subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
    ...
    @Override
    public void changeOrderStatus(final ReturnedGoodsCallback callback, String orderNumber, String status) {
        if (!LoginUtil.getInstance().isLogin()) {
